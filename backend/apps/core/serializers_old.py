...
class ProductSerializer(serializers.ModelSerializer):
    ...
    # Custom marketplace field that accepts any valid marketplace including Etsy
    marketplace = serializers.CharField(max_length=15)
    
    def validate_marketplace(self, value):
        """Custom validation to ensure marketplace is valid"""
        from .models import Product
        valid_choices = [choice[0] for choice in Product.ALL_MARKETPLACES]
        if value not in valid_choices:
            raise serializers.ValidationError(f"'{value}' is not a valid marketplace choice. Valid options: {valid_choices}")
        return value
    ...
    class Meta:
        ...
        # Exclude marketplace from automatic field generation to avoid ChoiceField creation
        extra_kwargs = {
            'marketplace': {'validators': []},  # Remove all validators
        }
